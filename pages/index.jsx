    ...
    <div className='h-screen flex justify-around items-center'>
      <div className='p-4 h-1/2'>
        <div className='flex items-center md:gap-10 gap-5 md:flex-row-reverse'>
          ...
          <div className='font-bold md:w-min w-2/3'>
            ...
            <p className='md:flex items-center gap-2 mt-5 hidden'>
              {links.map(e => (
                <a href={e.url} className='px-4 py-1 rounded-sm bg-indigo-600 transition duration-150 ease-in-out hover:bg-indigo-700'>
                  {e.title}
                </a>
              ))}
            </p>
        ...
        <div className='md:hidden flex items-center gap-2 mt-8 font-bold'>
          {links.map(e => (
            <a href={e.url} className='px-3 py-1 rounded-sm bg-indigo-600 transition duration-150 ease-in-out hover:bg-indigo-700 text-sm'>
              {e.title}
            </a>
          ))}
        </div>
